...
class Tutor(Teacher):
    ...
    def __init__(self, name, subject, department):
        # Nadpisanie wartosci pola nalezy dokonac po wywolaniu konstruktora klasy bazowej
        # Inaczej konstruktor klasy bazowej nadpisze nasza wartosc
        super().__init__(name, subject)
        self.guided_department = department
        self.assigned_departments = [department]
        # W przypadku, gdy dane pole jest lista, mozemy je zmodyfikowac/rozszerzyc np.dodajac wartosc
        # self.assigned_departments.append(department)

    def send_message_from_parents(self, message):
        print(f"Wiadomosc od rodzicow wysłana: '{message}'")
...
def run_example():
    tutor = Tutor(department="A2", name="Dominik", subject="Historia")
